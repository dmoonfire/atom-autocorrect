...
const ChangeableWord_1 = require("./ChangeableWord");
class AutoCorrectView {
    constructor(state, editor) {
         ...
         */
        this.editorSubscriptions = new Atom.CompositeDisposable();
        /**
        ...
        this.editorSubscriptions.add(editor.onDidChangeGrammar(() => { this.subscribe(); }));
        this.editorSubscriptions.add(atom.config.onDidChange("autocorrect.grammars", () => { this.subscribe(); }));
    ...
    processChange(change) {
        ...
        const wordChanges = this.getWordChanges(change.newRange);
    ...
    getWordChanges(range) {
        ...
        for (let rangeIndex = 0; rangeIndex < tokens.length; rangeIndex++) {
            ...
            const wordChange = new ChangeableWord_1.ChangeableWord(word, lastWordRange);
...
exports.AutoCorrectView = AutoCorrectView;
function lastIndexOf(input, search) {
...
//# sourceMappingURL=AutoCorrectView.js.map